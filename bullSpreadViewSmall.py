...
from localUtilities import errorHandler, configIB, buildOptionMatrices
...
class Ui_MainWindow(object):
    ...
    def get_underlying_info(self):
        ...
        if self.radioButton_Index.isChecked():
            a_underlying = Index(the_underlying, 'CBOE')
        else:
            a_underlying = Stock(the_underlying, 'SMART')
        ...
        if not get_underlying:  # empty list - failed qualifyContract
            ...
            self.statusbar.showMessage(str(get_underlying.pop()))
            buildOptionMatrices.qualify_index_option_chain()


    def onConnectButtonClicked(self):
        if self.ib.isConnected():
            self.ib.disconnect()
            self.table.clearTickers()  #needed to disconnect
